let handler  = async (m, { conn, args }) => {
  ...
  for (let id of groups) conn.sendMessage(id, args.join` ` + (/broadcast/im.test(args.join` `) ? '' : ('\n' + readMore + '[Broadcast GC [MDBOT]')), m.mtype, m.msg.contextInfo ? {
    contextInfo: m.msg.contextInfo
  } : {})
  conn.reply(m.chat, `_Mengirim pesan broadcast ke jika ada kendala Telegram:@WHIZQIDS ${groups.length} grup_`, m)
}
